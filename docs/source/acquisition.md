...
| `global_coordinate_system` | [CoordinateSystem](components/coordinates.md#coordinatesystem) or `Literal["NotApplicable"]` | Global coordinate system (Origin and axis definitions for determining the configured position of devices during acquisition, or `NotApplicable` when no coordinate data are present) |
