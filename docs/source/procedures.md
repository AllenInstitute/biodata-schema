...
| `global_coordinate_system` | [CoordinateSystem](components/coordinates.md#coordinatesystem) or `Literal["NotApplicable"]` | Global Coordinate System (Origin and axis definitions for determining the configured position of devices implanted during procedures, or `NotApplicable` when no coordinate data are present) |
